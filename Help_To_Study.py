...
def on_snap_shot(doc_snapshot, changes, read_time):
    for doc in doc_snapshot:
        ...
        # get users donations
        user = fs_db.collection('users').document('7ZdsBiu0g6Qj3ZuQrleh48Ehrd83').get().to_dict()
        # try:
        donations: List = user['donations']
        donations.append(donation)
        fs_db.collection('users').document('7ZdsBiu0g6Qj3ZuQrleh48Ehrd83').set({'donations': donations}, merge=True)
        print(str(donations))
        # except:
        #     pass
